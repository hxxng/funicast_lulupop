<?
include $_SERVER['DOCUMENT_ROOT']."/lib_inc.php";
include $_SERVER['DOCUMENT_ROOT']."/api_inc.php";
use \Firebase\JWT\JWT;

if($decoded_array['debug_jwt'] && ($decoded_array['debug_jwt']!=DEBUG_JWT)){
    echo result_data('false', '정상적인 접근이 아닙니다. DEBUG_JWT', '');
    exit();
}
if($decoded_array['mt_id']=="") {
    echo result_data('false', '잘못된 접근입니다. mt_id', '');
    exit;
}
if($decoded_array['ct_idx']=="") {
    echo result_data('false', '잘못된 접근입니다. ct_idx', '');
    exit;
}

$query_m = "select *, a1.idx as mt_idx from member_t a1 where mt_level = 3 and mt_id = '".$decoded_array['mt_id']."'";
$row_m = $DB->fetch_query($query_m);
if($row_m['mt_idx']) {
    $query = "select * from community_t where mt_idx = ".$row_m['mt_idx']." and idx = ".$decoded_array['ct_idx'];
    $row = $DB->fetch_assoc($query);
    if($row['idx']) {
        if($row['ct_comment_chk'] == "Y") {
            $ct_comment_chk = "N";
            $txt = "해제";
        } else {
            $ct_comment_chk = "Y";
            $txt = "사용";
        }
        $DB->update_query("community_t", array("ct_comment_chk" => $ct_comment_chk), " idx = ".$decoded_array['ct_idx']);

        $payload['data'] = $arr;

        if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
            echo result_data('true', '[debug] 게시글 댓글 기능 '.$txt, $payload);
        } else {
            $jwt = JWT::encode($payload, $secret_key);
            echo result_data('true', '게시글 댓글 기능 '.$txt, $jwt);
        }
    } else {
        echo result_data("false", "본인이 작성한 게시글이 아닙니다.", "");
        exit;
    }
} else {
    echo result_data('false', '존재하지 않는 회원정보입니다.', $arr);
}

include $_SERVER['DOCUMENT_ROOT']."/tail_inc.php";
?>