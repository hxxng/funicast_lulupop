...
if($row['mt_idx']) {
    if($row['mt_level'] == 3) {
        ...
        $query = "select * from order_t left join cart_t on cart_t.ot_code = order_t.ot_code where ct_status in (7,8) and order_t.mt_idx = " . $row['mt_idx'] . " and cart_t.ot_pcode = '" . $decoded_array['ot_pcode'] . "'";
        ...
        if ($list_ot) {
            ...
            if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
                echo result_data('true', '[debug] 취소 상세보기', $payload);
            } else {
                $jwt = JWT::encode($payload, $secret_key);
                echo result_data('true', '취소 상세보기', $jwt);
            }
        } else {
        ...
        $query = "select * from order_t left join cart_t on cart_t.ot_code = order_t.ot_code where ct_status in (7,8) and order_t.nmt_id = '" . $decoded_array['mt_id'] . "' and cart_t.ot_code = '" . $decoded_array['ot_code'] . "'";
        $sql_query = $query . " order by ot_wdate desc";
        $list_ot = $DB->select_query($sql_query);
        $list_fetch = $DB->fetch_assoc($sql_query);
        if ($list_ot) {
