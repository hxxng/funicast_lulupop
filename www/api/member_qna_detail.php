        ...
        if($row['pt_idx']) {
            ...
            if($product['idx']) {
                $product = array(
                    ...
                    "pt_image" => $ct_img_url."/".$product['pt_image1'],
                );
            }

            $arr['qna']['product'] = $product;
            $title = "나의";
        } else {
            $title = "관리자";
            $arr['qna']['product'] = null;
        }

        $payload['data'] = $arr;

        if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
            echo result_data('true', '[debug] '.$title.' 문의 상세보기', $payload);
        } else {
            $jwt = JWT::encode($payload, $secret_key);
            echo result_data('true', $title.' 문의 상세보기', $jwt);
        }
    } else {
