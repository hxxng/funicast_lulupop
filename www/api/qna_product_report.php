...
if($row_m['mt_idx']) {
    ...
    if($row['idx']) {
        $query = "select * from report_t where rt_type = 2 and rt_table = 'qna_t' and reporter_idx = " . $row_m['mt_idx'] . " and report_idx = " . $decoded_array['qt_idx'];
        ...
        if ($count > 0) {
            ...
            $arr = array(
                ...
                "report_idx" => $decoded_array['qt_idx'],
        ...
        if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
            echo result_data('true', '[debug] 상품 문의 신고', $payload);
        } else {
            $jwt = JWT::encode($payload, $secret_key);
            echo result_data('true', '상품 문의 신고', $jwt);
        }
    } else {
        echo result_data("false", "신고할 문의가 존재하지 않습니다.", "");
        exit;
    }
} else {
