...
if($row['mt_idx']) {
    if($row['mt_level'] == 3) {
        $qq = 0;
        unset($list_ot);
        $query_ot = "select * from order_t left join cart_t on cart_t.ot_code = order_t.ot_code where ct_select = 2 and ct_status > 0 and ct_status in (7,8) and 
        ...
        $count = $DB->select_query("select * from order_t left join cart_t on cart_t.ot_code = order_t.ot_code where ct_select = 2 and ct_status > 0 and ct_status in (7,8) and order_t.mt_idx = " . $row['mt_idx']);
        if($count) {
            $count = count($count);
        ...
        if ($list_ot) {
            foreach ($list_ot as $row_ot) {
                $query_pt = "select * from order_t left join cart_t on cart_t.ot_code = order_t.ot_code where ct_status in (7,8) and order_t.mt_idx = " . $row['mt_idx'] . " and order_t.ot_code = '" . $row_ot['ot_code'] . "'";
            ...
            if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
                echo result_data('true', '[debug] 취소내역리스트', $payload);
            } else {
                $jwt = JWT::encode($payload, $secret_key);
                echo result_data('true', '취소내역리스트', $jwt);
            }
        } else {
        ...
        $list_ot = $DB->select_query($query_ot);
        $count = $DB->select_query("select * from order_t left join cart_t on cart_t.ot_code = order_t.ot_code where ct_select = 2 and ct_status > 0 and ct_status in (7,8) and order_t.nmt_id = '" . $decoded_array['mt_id'] . "' group by order_t.ot_code");
        if($count) {
            $count = count($count);
            $n_page = ceil($count / 20);
        } else {
        ...
        if ($list_ot) {
            foreach ($list_ot as $row_ot) {
                $query_pt = "select * from order_t left join cart_t on cart_t.ot_code = order_t.ot_code where ct_status in (7,8) and order_t.nmt_id = '" . $decoded_array['mt_id'] . "' and order_t.ot_code = '" . $row_ot['ot_code'] . "'";
                $list_pt = $DB->select_query($query_pt);
            ...
            if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
                echo result_data('true', '[debug] 비회원 취소내역리스트', $payload);
            } else {
                $jwt = JWT::encode($payload, $secret_key);
                echo result_data('true', '비회원 취소내역리스트', $jwt);
            }
        } else {
            echo result_data('false', '주문내역이 없습니다.', $arr);
        }
