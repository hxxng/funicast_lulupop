...
if($row_m['mt_idx']) {
    $query = "select * from community_t where idx = ".$decoded_array['ct_idx']." and ct_status = 1";
    $row = $DB->fetch_assoc($query);
    if($row['idx'] > 0) {
        $query = "select * from report_t where rt_type = 2 and rt_table = 'community_t' and reporter_idx = ".$row_m['mt_idx']." and report_idx = ".$decoded_array['ct_idx'];
        ...
        if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
            echo result_data('true', '[debug] 커뮤니티 게시글 신고', $payload);
        } else {
            $jwt = JWT::encode($payload, $secret_key);
            echo result_data('true', '커뮤니티 게시글 신고', $jwt);
        }
    } else {
