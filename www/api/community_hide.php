...
if($row_m['mt_idx']) {
    ...
    if($row['idx']) {
        $query = "select * from hide_t where ht_hide_idx = ".$decoded_array['ct_idx']." and ht_table = 'community_t' and mt_idx = ".$row_m['mt_idx'];
        $count = $DB->count_query($query);
        if($count > 0) {
            $DB->del_query("hide_t", " ht_table = 'community_t' and mt_idx = ".$row_m['mt_idx']." and ht_hide_idx = ".$decoded_array['ct_idx']);
            $arr = $row;
            ...
            $DB->insert_query("hide_t", array("ht_table" => "community_t", "mt_idx" => $row_m['mt_idx'], "ht_hide_idx" => $decoded_array['ct_idx'], "ht_wdate" => "now()"));
            $arr = $row;

            $payload['data'] = $arr;

            if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
                echo result_data('true', '[debug] 게시글 숨김', $payload);
            } else {
                $jwt = JWT::encode($payload, $secret_key);
                echo result_data('true', '게시글 숨김', $jwt);
            }
        }
    } else {
