...
if($decoded_array['rpt_idx']=="") {
    echo result_data('false', '잘못된 접근입니다. rpt_idx', '');
    exit;
}
...
if($row_m['mt_idx']) {
    $query = "select * from review_product_t where idx = ".$decoded_array['rpt_idx'];
    $row = $DB->fetch_assoc($query);
    if($row['idx']) {
        $query = "select * from report_t where rt_type = 2 and rt_table = 'review_product_t' and reporter_idx = ".$row_m['mt_idx']." and report_idx = ".$decoded_array['rpt_idx'];
        ...
        if($count > 0) {
            ...
            $arr = array(
                ...
                "report_idx" => $decoded_array['rpt_idx'],
                "rt_content" => $decoded_array['rt_content'],
                "rt_table" => "review_product_t",
                "rt_wdate" => "now()",
            );
        ...
        if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
            echo result_data('true', '[debug] 상품 후기 신고', $payload);
        } else {
            $jwt = JWT::encode($payload, $secret_key);
            echo result_data('true', '상품 후기 신고', $jwt);
        }
    } else {
        echo result_data("false", "신고할 후기가 존재하지 않습니다.", "");
        exit;
    }
} else {
