...
if($decoded_array['hashtag']=="") {
    echo result_data('false', '잘못된 접근입니다. hashtag', '');
    exit;
}
...
if($row_m['mt_idx']) {
    $query = "select *, (SELECT mt_nickname FROM member_t WHERE idx=mt_idx) as mt_nickname from community_t 
        ...
        and instr(ct_hashtag, '".$decoded_array['hashtag']."')";
} else {
    $query = "select *, (SELECT mt_nickname FROM member_t WHERE idx=mt_idx) as mt_nickname from community_t 
        where ct_status = 1 
        and instr(ct_hashtag, '".$decoded_array['hashtag']."')";
...
if($list) {
    ...
    if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
        echo result_data('true', '[debug] 해시태그 검색', $payload);
    } else {
        $jwt = JWT::encode($payload, $secret_key);
        echo result_data('true', '해시태그 검색', $jwt);
    }
} else {
