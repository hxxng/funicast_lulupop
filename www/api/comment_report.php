...
if($row_m['mt_idx']) {
    ...
    if($row['idx'] > 0) {
        $query = "select * from report_t where rt_type = 2 and rt_table = 'comment_t' and reporter_idx = ".$row_m['mt_idx']." and report_idx = ".$decoded_array['ct_idx'];
        $count = $DB->count_query($query);
        if($count > 0) {
            echo result_data("false", "이미 신고 처리한 댓글입니다.", "");
            exit;
        } else {
            $arr = array(
                "rt_type" => 2,
                ...
                "report_idx" => $decoded_array['ct_idx'],
...
//            $DB->update_query("comment_t", array("ct_status" => 2), " idx = ".$decoded_array['ct_idx']);
        }

        $payload['data'] = $arr;

        if ($decoded_array['debug_jwt'] == DEBUG_JWT) {
            echo result_data('true', '[debug] 댓글 신고', $payload);
        } else {
            $jwt = JWT::encode($payload, $secret_key);
            echo result_data('true', '댓글 신고', $jwt);
        }
    } else {
        echo result_data("false", "신고할 댓글이 존재하지 않습니다.", "");
        exit;
    }
} else {
