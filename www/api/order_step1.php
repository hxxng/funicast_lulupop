    ...
    if($list) {
        foreach($list as $row_c) {
            ...
            if($product['pt_stock_chk'] == "Y") {
                if($product['pt_option_chk'] == 2) {
                    $query = "select * from product_option_t where pt_idx = ".$row_c['pt_idx'];
                    $option = $DB->select_query($query);
                    if($option) {
                        foreach ($option as $row_o) {
                            if($row_o['pot_value'] == $row_c['ct_opt_value']) {
                                if ($row_c['ct_opt_qty'] > $row_o['pot_jaego']) {  //재고부족
                                    echo result_data('false', "재고가 부족합니다. 현재 재고수량 : " . (int)$row_o['pot_jaego'], "");
                                    exit;
                                }
                            }
                        }
...
//    $field_arr['ct_delivery_price_add'] = $pt_plus_delivery_price;	//추가배송비
...
if($decoded_array['ot_pay_type']) $arr_query['ot_pay_type'] = $decoded_array['ot_pay_type'];
...
//if($pt_plus_delivery_price) $arr_query['odt_delivery_add_price'] = $pt_plus_delivery_price;
...
//기본배송지등록
//if($decoded_array['default_shipping_address_check']=='on'){
//    $member_set = array(
//        'mt_recipient' => $decoded_array['odt_rname'],
//        'mt_tel' => $decoded_array['odt_rtel'],
//        'mt_phone' => $decoded_array['odt_rhp'],
//        'mt_zip' => $decoded_array['odt_rzip'],
//        'mt_add1' => $decoded_array['odt_radd1'],
//        'mt_add2' => $decoded_array['odt_radd2']
//    );
//    $DB->update_query('member_t', $member_set, " idx = ".$mt_idx);
//}

$payload['data']['ot_code'] = $ot_code;
$payload['data']['ot_pt_name'] = $ot_pt_name;
