<?
	...
	$chk_menu = "4";
    $list_url_t = "community_list.php";

	$chk_post_code = 'Y';
	include "./head_menu_inc.php";

    $query = "
        select *, a1.idx as ct_idx, (select mt_nickname from member_t where idx=mt_idx) as mt_nickname, (SELECT pc_name FROM product_category_t WHERE pc_depth = 0 and idx=ct_cate_idx) as pc_name from community_t a1
...
<div class="content-wrapper">
	<div class="row">
		<div class="col-md-12 grid-margin stretch-card">
			<div class="card">
                <form method="post" name="frm_form" id="frm_form" action="./community_update.php" onsubmit="return frm_form_chk(this);" target="hidden_ifrm">
                <input type="hidden" name="act" value="chg_status" />
                <input type="hidden" name="idx" value="<?=$row['ct_idx']?>" />
                <div class="card-body">
                    <div class="row">
                        <h4 class="card-title">커뮤니티 게시글 페이지</h4>
                        <div class="col-sm-5 ml-4">
                            <div class="custom-control custom-radio custom-control-inline">
                                <input type="radio" id="normal" name="ct_status" <? if($row['ct_status'] == 1) echo "checked=''";?> value="1" class="custom-control-input">
                                <label class="custom-control-label" for="normal">정상</label>
                            </div>
                            <div class="custom-control custom-radio custom-control-inline">
                                <input type="radio" id="blind" name="ct_status" <? if($row['ct_status'] == 2) echo "checked=''";?> value="2" class="custom-control-input">
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="row align-items-center" style="padding-left: 10px;">
                                    ...
                                    <div class="col-sm-9 custom-control">
                                        <span><?=$row['pc_name']?></span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="row align-items-center" style="padding-left: 10px;">
                                    <label for="mt_id" class="col-sm-3 col-form-label">좋아요누적</label>
                                    <div class="col-sm-9 custom-control">
                                        <span><?=$row['ct_like']?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <li class="list-group-item" style="border: none;">
                                            <div class="form-group row align-items-center mb-0">
                                                <div class="col-sm-12 row">
                                                    <? for($q=1;$q<=$pt_image_num;$q++) {  if($row['ct_img'.$q] != "") {?>
                                                        <div class="media ">
                                                            <a href="javascript:;" onclick="f_popup_image('<?=$row['idx']?>', '<?=$q?>')"><img src="<?=$ct_img_url."/".$row['ct_img'.$q].'?v='.time()?>" onerror="this.src='<?=$ct_no_img_url?>'" class="align-self-center mr-3" alt="<?=$q?>" style="width: 150px; height: 150px;border-radius: 10px;"></a>
                            ...
                            <div class="col-sm-12">
                                <div class="row align-items-center" style="padding-left: 10px;">
                                    ...
                                    <div class="custom-control" style="width: 1338px;height: 200px;overflow-y: auto;">
                                        <span class="p-1" style="background-color: #fff;border: none;"><?=$row['ct_content']?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    ...
                    where ct_table = 'community_t' and ct_idx = ".$row['ct_idx']." and mt_id is not null order by IF(ISNULL(ct_parent_idx), comment_t.idx, ct_parent_idx) ";
...
<script>
    ...
    function f_popup_image(idx, num) {
        $.post('./community_update.php', {act: 'popup_image', idx: idx, num: num}, function (data) {
            if(data) {
