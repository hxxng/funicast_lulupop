                        ...
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item">
                                <div class="form-group row align-items-center mb-0">
                                    <label for="search_txt" class="col-sm-2 col-form-label">검색어</label>
                                    <div class="col-sm-4">
                            ...
                            <li class="list-group-item">
                                <div class="form-group row align-items-center mb-0">
                                    <label for="sel_search_date" class="col-sm-2 col-form-label">주문일</label>
                                    <div class="col-sm-4">
                                        <div class="input-group">
                            ...
                            <li class="list-group-item">
                                <div class="form-group row align-items-center mb-0">
                                    <div class="col-sm-12 text-center">
                                        ...
                                        <input type="button" class="btn btn-secondary ml-2" value="초기화" onclick="location.href='./order_list.php'" />
                                    </div>
                                </div>
                            </li>
                        </ul>
...
<!--                                        <button type="button" onclick="confirm_deposit();" class="btn btn-outline-primary btn-sm">입금처리</button>-->
<!--                                    </div>-->
<!--                                </div>-->
<!--                            </li>-->
<!--                        </ul>-->
...
<!--							<input type="button" class="btn btn-secondary btn-xs" value="선택" onclick="f_checkbox_all('chk_all')" />-->
					...
					<?
                        $_where = " and ";
						$query = "
                            select a1.* from order_t a1 where a1.ot_status in (1,2,3,4,5,6)
						";
						if($_GET['search_txt']) {
                        ...
                        if($_GET['sel_search_sdate'] && $_GET['sel_search_edate']) {
                            $where_query .= $_where." ot_wdate between '".$_GET['sel_search_sdate']." 00:00:00' and '".$_GET['sel_search_edate']." 23:59:59'";
                            $_where = " and ";
                        }
						...
						unset($list);
						$sql_query = $query.$where_query." order by a1.ot_wdate desc limit ".$n_from.", ".$n_limit;
...
//								$pt_info = get_product_t_info($row['pt_idx']);
					...
					<tr>
						...
						<td class="text-center" style="width: 80px;">
                            <?=$arr_ct_status[$row['ot_status']]?>
						</td>
						<td class="text-center">
							<input type="button" class="btn btn-outline-secondary btn-sm" value="자세히" onclick="location.href='./order_form.php?act=view_order&ot_code=<?=$row['ot_code']?>&<?=$_get_txt.$_GET['pg']?>'" />
						</td>
					</tr>
					<?
								$counts--;
    ...
    function confirm_deposit() {
        ...
        if (confirm("입금 처리하시겠습니까?")) {
            $.ajax({
                ...
                dataType: 'json',
                data: {act: 'confirm_deposit', idx: idx},
