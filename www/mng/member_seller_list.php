				...
				<div class="card-body">
					...
					<div class="p-3 float-right">
						<form method="get" name="frm_search" id="frm_search" action="<?=$_SERVER['PHP_SELF']?>" class="form-inline" onsubmit="return frm_search_chk(this);">
							...
							<div class="form-group mx-sm-1">
								<input type="button" class="btn btn-secondary" value="초기화" onclick="location.href='./member_seller_list.php'" />
							</div>
						</form>
					...
					<?
						...
						if($list) {
							foreach($list as $row) {
								$query_st = "select * from seller_t where mt_idx = '".$row['mt_idx']."'";
								$row_st = $DB->fetch_query($query_st);
					?>
					<tr>
						...
						<td class="text-center">
							<?=$row_st['slt_company_hp1']?>
						</td>
						<td class="text-center">
							<?=$row_st['slt_company_name']?>
						</td>
						<td class="text-center">
							<?=$row_st['slt_company_num']?>
						</td>
						<td class="text-center">
							<input type="button" class="btn btn-outline-primary btn-sm" value="수정" onclick="location.href='./member_form.php?act=update&form_type=S&mt_idx=<?=$row['mt_idx']?>&<?=$_get_txt.$_GET['pg']?>'" /> <input type="button" class="btn btn-outline-danger btn-sm" value="탈퇴" onclick="f_retire_mem('<?=$row['mt_idx']?>');" />
						</td>
					</tr>
					<?
								$counts--;
