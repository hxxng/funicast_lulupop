<?
    ...
    <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">랜덤 상품 추가</h5>
...
<script>
    ...
    <?
    } else if($_POST['act']=='get_list') {
        $where = " where pt_show = 'Y' and pt_sale_now = 'Y' ";
        ...
        if($list) {
            foreach($list as $row) {
                ...
                $result['html'] .= '</td>';
                $result['html'] .= '<td class="text-center">';
                $result['html'] .= '   <input type="button" class="btn btn-outline-primary btn-sm" value="추가" onclick="add_product('.$row['pt_idx'].')">';
                $result['html'] .= '</td>';
                $result['html'] .= '</tr>';
            }
        } else {
        ...
        if($n_page>1) {
            $result['page'] = pageing_list_ajax($pg, $n_page, $_SERVER['PHP_SELF'],"get_list");
        }else{
            $result['page'] = "";
        }
        ...
        $pt = $DB->fetch_assoc("select pt_random_chk from product_t where idx = ".$_POST['pt_idx']);
        if($pt['pt_random_chk'] == "Y") {
            echo json_encode(array('result' => 'false', 'msg' => "이미 랜덤 등록된 상품입니다."));
        } else {
            $DB->update_query("product_t", array("pt_random_chk" => "Y"), "idx = ".$_POST['pt_idx']);
            $DB->insert_query("catalog_t", array("pt_idx" => $_POST['pt_idx'], "ct_udate" => "now()"));
        ...
        {
            $DB->update_query('product_t', array("pt_random_chk" => "N", 'pt_random_percentage' => null), "idx = ".$_POST['pt_idx'][$i]);
            $DB->del_query("catalog_t", "pt_idx = ".$_POST['pt_idx'][$i]);
        }
        echo json_encode(array('result' => '_ok', 'msg'=>'삭제되었습니다.'));
    } else if($_POST['act'] == 'sel_effect') {
        for($i=0; $i<count($_POST['pt_idx']); $i++)
        {
            $DB->update_query('product_t', array("pt_random_effect" => $_POST['effect']), "idx = ".$_POST['pt_idx'][$i]);
        }
        echo json_encode(array('result' => '_ok', 'msg'=>'변경되었습니다.'));
    } else if($_POST['act'] == 'save_percentage') {
        for($i=0; $i<count($_POST['pt_idx']); $i++) {
            $DB->update_query("product_t", array("pt_random_percentage" => $_POST['pt_random_percentage'][$i]), " idx = ".$_POST['pt_idx'][$i]);
        }
